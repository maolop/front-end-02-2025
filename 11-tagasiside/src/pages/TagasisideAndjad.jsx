...
function TagasisideAndjad() {
	...
	return (
		<>
			...
			<button onClick={() => setTagasisideAndjad(tagasisideAndjad.filter((i) => i.startsWith("M")))}>
				Näita M-iga algavaid
			</button>
			<button onClick={() => setTagasisideAndjad(tagasisideAndjad.filter((i) => i.length === 6))}>
				Näita 6-tähelisi
			</button>
			<button onClick={() => setTagasisideAndjad(tagasisideAndjad.filter((i) => i.endsWith("y")))}>
				Näita y-ga lõppevaid
			</button>
			<button onClick={() => setTagasisideAndjad(tagasisideAndjad.toSorted((a, b) => b.localeCompare(a)))}>
				Sorteeri Z-A
			</button>
			<br />
			...
			{tagasisideAndjad.map((name, index) => (
				<span
					className="name"
					onClick={() => setTagasisideAndjad(tagasisideAndjad.filter((a) => a != name))}
					key={name}
				>
					{index + 1}. {name}{" "}
				</span>
